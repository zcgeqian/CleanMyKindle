...
class CF(Frame):
    ...
    def initUI(self):
        ...
        # 默认文件路径
        self.filePath = 'H:/'  # Mac 下默认 Kindle 路径
        ...
        self.btnSelect = Button(
            self, text=btnSlectTxt(), command=self.onSelect)
        ...
        if(self.filePathVer):
            ...
            self.entry.insert(END, '未检测到 Kindle 磁盘，请手动选择')  # 不存在显示

    def onSelect(self):
        # 调用打开对话框
        self.filePath = tkinter.filedialog.askdirectory(
            title='请选择 Kindle 磁盘...')
        # 如果确认选择
        if self.filePath:
            self.entry.delete(0, END)  # 清空路径
            self.entry.insert(END, self.filePath)  # 填入路径
            if self.btnSelect['text'] != '重选':
                self.btnSelect['text'] = '重选'
    ...
    def onProcess(self):
        ...
        if checkDocumentsPathVer == False:
            ...
            if clean == True:
                ...
                # 清理成功
                self.doneAlert(sdr_s_count_str, sdr_f_count_str,
                               dir_s_count_str, dir_f_count_str, shot_count_str)
                # 清理日志
                if self.var.get() == 1:
                    ...
                    if sdr_list_s:
                        new_log.write(
                            '共删除成功 ' + sdr_s_count_str + ' 个无用 SDR 文件夹：\n---------------------------------\n')
                        new_log.write(sdr_list_s)
                        new_log.write('=================================\n')
                    if sdr_list_f:
                        new_log.write(
                            '有 ' + sdr_f_count_str + ' 个 SDR 文件夹没有删除成功：\n---------------------------------\n')
                        new_log.write(sdr_list_f)
                        new_log.write('=================================\n')
                    if dir_list_s:
                        new_log.write(
                            '共删除成功 ' + dir_s_count_str + ' 个无用 DIR 文件夹：\n---------------------------------\n')
                        new_log.write(dir_list_s)
                        new_log.write('=================================\n')
                    if dir_list_f:
                        new_log.write(
                            '有 ' + dir_f_count_str + ' 个 DIR 文件夹没有删除成功：\n---------------------------------\n')
                        new_log.write(dir_list_f)
                        new_log.write('=================================\n')
                    if sdr_list_f or dir_list_f:
                        new_log.write(
                            '* 删除失败是因为该文件名中含有特殊字符无法被删除，目前还没找到好的办法解决，请暂时手动删除。\n')
                        new_log.write('---------------------------------\n')
                    if scs_list:
                        new_log.write('共删除成功 ' + shot_count_str +
                                      ' 个截图：\n---------------------------------\n')
